...
class RTSPSafeRecognizer:
    def __init__(self, rtsp_urls, person_model, safe_model, interval=20):
        """
        Args:
    ...
    def start(self):
        ...
        with ThreadPoolExecutor(max_workers=len(self.rtsp_urls)) as executor:
            for rtsp_url in self.rtsp_urls:
                # 安全帽和反光衣识别
                safe_recognizer = CrewStaffSecurityRecognizer(self.person_model, self.safe_model)
...
if __name__ == "__main__":
    ...
    person_model = "/Users/jinyfeng/tools/object-det/yolo11s.pt"
    safe_model = "/Users/jinyfeng/tools/best.pt"

    print("Starting RTSP Stream Recognizer...")
    print(f"Started processing {len(rtsp_urls)} streams")

    recognizer = RTSPSafeRecognizer(rtsp_urls, person_model, safe_model, interval=20)
    recognizer.start()